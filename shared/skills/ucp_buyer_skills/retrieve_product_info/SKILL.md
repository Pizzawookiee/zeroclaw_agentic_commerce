...
name: Retrieve Merchant Product Info
description: Retrieve product or variant details from a named merchant agent through the local A2A gateway and return the merchant's UCP-shaped product lookup response.
tags:
  - ucp_buyer_skills
  - commerce
  - catalog
...
# Retrieve Merchant Product Info

## Purpose

Use this skill when the buyer wants product or variant details from a merchant agent through A2A.

This skill calls another agent's `retrieve_product_info` skill through the ZeroClaw gateway:
...
The caller may provide a merchant agent name and product or variant ID in natural language:

```text
Ask merchant my_merchant_agent for product prod_headphones_001
```
...
Product: prod_headphones_001
```

If the merchant agent name was not provided, ask the user for the merchant agent name before sending the A2A request.

If the product or variant identifier was not provided, ask the user for it before sending the A2A request.

## Required Behavior

1. Extract the merchant agent name from the user request when present.
2. Extract the product ID or variant ID from the user request when present.
3. If the merchant agent name is missing, ask the user for it and wait for the answer.
4. If the product or variant identifier is missing, ask the user for it and wait for the answer.
...
7. Substitute the extracted product or variant identifier into the message text.
8. Ask the remote merchant to return only valid JSON in the UCP-shaped product lookup response format.
  ...
  "params": {
    "message": {
      ...
      "parts": [
        {
          "kind": "text",
          "text": "Use the retrieve_product_info skill. Product or variant ID: <product_or_variant_id>. Return only valid JSON in the UCP-shaped product lookup response format."
        }
      ]
    }
  }
...
curl -sS \
  ...
  --data-raw '{"jsonrpc":"2.0","id":1,"method":"message/send","params":{"message":{"role":"user","parts":[{"kind":"text","text":"Use the retrieve_product_info skill. Product or variant ID: <product_or_variant_id>. Return only valid JSON in the UCP-shaped product lookup response format."}]}}}'
```
  ...
  "messages": [
    {
      ...
      "content": "The merchant A2A product lookup request failed. <runtime error if available>"
    }
  ]
}
```
